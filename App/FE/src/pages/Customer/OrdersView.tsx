      ...
      <div className="space-y-6 pb-10 animate-in fade-in slide-in-from-right-4 duration-300">
        ...
        <div className="grid grid-cols-2 gap-3">
          <Card className="p-4 border-neutral-100 dark:border-neutral-800 rounded-xl shadow-none bg-neutral-50/50 dark:bg-neutral-900/50">
            ...
            <p className="text-neutral-400 dark:text-neutral-500 text-xs">{t("ov_label_date")}</p>
            <p className="text-sm font-bold text-neutral-800 dark:text-neutral-200">{dayjs(selectedOrder.created_at).format("DD MMM YYYY")}</p>
          </Card>
          <Card className="p-4 border-neutral-100 dark:border-neutral-800 rounded-xl shadow-none bg-neutral-50/50 dark:bg-neutral-900/50">
            <CreditCard size={16} className="text-red-600 mb-2" />
            <p className="text-neutral-400 dark:text-neutral-500 text-xs">{t("ov_label_method")}</p>
            <p className="text-sm font-bold text-neutral-800 dark:text-neutral-200 uppercase">{selectedOrder.payment_method || "QRIS"}</p>
          </Card>
        </div>
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-2 px-1">
            ...
            <h3 className="text-sm font-bold text-neutral-800 dark:text-neutral-200">{t("ov_section_items")}</h3>
          </div>
                ...
                <div key={idx} className="p-4 flex justify-between items-center bg-white dark:bg-neutral-900">
                  <div className="flex gap-3 items-center">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-neutral-400">{item.quantity}x Rp {new Intl.NumberFormat("id-ID").format(item.price)}</p>
                    </div>
                  </div>
                  <p className="text-sm font-bold text-neutral-800 dark:text-neutral-200">Rp {new Intl.NumberFormat("id-ID").format(item.subtotal)}</p>
                </div>
              ))}
            </div>
          </Card>
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-2 px-1">
            ...
            <h3 className="text-sm font-bold text-neutral-800 dark:text-neutral-200">{t("ov_section_bill")}</h3>
          </div>
          <Card className="p-5 border-neutral-100 dark:border-neutral-800 rounded-lg shadow-none bg-white dark:bg-neutral-900 space-y-3">
            <div className="flex justify-between text-sm">
              ...
              <span className="font-bold text-neutral-800 dark:text-neutral-200">Rp {new Intl.NumberFormat("id-ID").format(subtotal)}</span>
            </div>
            {isServiceActive && (
              <div className="flex justify-between text-sm">
                <span className="text-neutral-400">{t("ov_bill_service")} ({servicePercent}%)</span>
                <span className="font-bold text-neutral-800 dark:text-neutral-200">Rp {new Intl.NumberFormat("id-ID").format(serviceAmount)}</span>
              </div>
            )}
            {isTaxActive && (
              <div className="flex justify-between text-sm">
                <span className="text-neutral-400">{t("ov_bill_tax")} ({taxPercent}%)</span>
                <span className="font-bold text-neutral-800 dark:text-neutral-200">Rp {new Intl.NumberFormat("id-ID").format(taxAmount)}</span>
              </div>
            )}
            <div className="pt-3 border-t border-dashed border-neutral-200 dark:border-neutral-700 flex justify-between items-center">
              ...
              <span className="text-xl font-black text-red-600">Rp {new Intl.NumberFormat("id-ID").format(grandTotal)}</span>
            </div>
          </Card>
        </div>
      </div>
        ...
        <div key={order.id} onClick={() => setSelectedOrder(order)} className="cursor-pointer">
          <Card className="p-4 border-neutral-100 dark:border-neutral-800 shadow-none rounded-2xl bg-white dark:bg-neutral-900 hover:border-red-200 dark:hover:border-red-900/50 hover:shadow-md hover:shadow-red-500/5 transition-all group">
            <div className="flex justify-between items-start mb-2">
              <div>
                <p className="text-[10px] text-neutral-400 font-mono mb-1">#{order.transaction_code}</p>
                <h4 className="text-sm font-bold text-neutral-800 dark:text-neutral-200 leading-tight group-hover:text-red-600 transition-colors">
                  {order?.details?.length > 0 ? order?.details[0]?.menu?.name : `Order #${order.id}`}
                  {order?.details?.length > 1 && ` +${order.details.length - 1} ${t("ov_item_more")}`}
                </h4>
              </div>
              <span className={`text-[9px] px-2 py-0.5 rounded-full font-bold uppercase ${["paid", "completed"].includes(order.status) ? "bg-green-50 text-green-600 dark:bg-green-950/30" : "bg-yellow-50 text-yellow-600 dark:bg-yellow-950/30"}`}>
                {order.status}
              </span>
            </div>
