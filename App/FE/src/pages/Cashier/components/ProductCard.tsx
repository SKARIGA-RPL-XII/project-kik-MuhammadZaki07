  ...
  export function ProductCard({ item, onClick }: ProductCardProps) {
    ...
    return (
      <TooltipProvider>
        <Card
          className={`group relative shadow-none border-2 transition-all duration-300 rounded-xl bg-white dark:bg-neutral-800 overflow-hidden select-none
            ${
              isOutOfStock
          ...
          <CardContent className="p-0">
            ...
            <div className="p-3 space-y-4.5">
              <div className="flex flex-col">
                <span className="text-xs text-zinc-400 leading-none mb-2">
                  {item.category?.name || "Menu"}
                </span>
                <h3 className="font-bold text-zinc-900 dark:text-neutral-200 text-lg leading-tight truncate group-hover:text-red-600 transition-colors">
                  {item.name}
                </h3>
              </div>
