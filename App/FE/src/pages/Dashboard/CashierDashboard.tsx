...
export default function CashierDashboard() {
  ...
  return (
    <>
      <PageMeta title="Cashier Dashboard | Gagal-Lapar" description="Manage transactions efficiently." />
...
function StatCard({ title, value, icon }: { title: string; value: any; icon: any }) {
  return (
    <Card className="p-5 rounded-xl shadow-none border flex items-center gap-4">
      <div className="p-3 bg-neutral-50 dark:bg-neutral-800 rounded-lg">{icon}</div>
      <div>
        <p className="text-sm text-neubg-neutral-500 dark:text-neubg-neutral-400 font-medium">{title}</p>
        <h3 className="text-2xl font-bold text-neubg-neutral-900 dark:text-white">{value}</h3>
      </div>
    </Card>
  );
}