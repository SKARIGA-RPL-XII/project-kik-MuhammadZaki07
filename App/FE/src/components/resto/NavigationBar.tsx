   ...
   <div className="sticky top-0 z-40 bg-[#fcfcfc] dark:bg-neutral-900 pb-3 space-y-4 border-b px-4 pt-4">
      <div className="w-full">
        <div className="flex flex-col lg:flex-row lg:items-center gap-3">
          <div className="flex items-center gap-2 w-full lg:flex-1">
            <div className="relative flex-1 lg:max-w-md">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-3 rounded-sm bg-white border dark:bg-neutral-800 focus:bg-white dark:text-neutral-300 focus:border-red-500 transition-all outline-none text-sm text-neutral-900"
              />
            </div>
              ...
              <button
                ...
                className={`relative flex items-center justify-center w-11 h-11 border rounded-sm transition-all ${
                  selectedSorts.length > 0
                    ? "bg-red-50 border-red-500 dark:bg-neutral-800 text-red-600"
                    : "bg-white dark:bg-neutral-800 text-neutral-600 hover:bg-neutral-50"
                }`}
              >
                      ...
                      <div className="p-2 space-y-1">
                        {sortOptions.map((opt) => (
                          <button
                            ...
                            className="w-full flex items-center gap-3 px-3 py-2.5 rounded-sm text-sm transition-colors hover:bg-neutral-50 dark:hover:bg-neutral-800 group text-left"
                          >
                            <div
                            ...
                            <span className={`text-xs ${selectedSorts.includes(opt.id) ? "font-bold text-neutral-900 dark:text-neutral-100" : "text-neutral-600 dark:text-neutral-400"}`}>
                              {opt.label}
                            </span>
                          </button>
                        ))}
          ...
          {!isBookingPage && (
            <Button
              className="bg-red-500 hover:bg-red-600 text-white w-full lg:w-auto px-8 h-11 rounded-sm shadow-md"
      ...
      <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide flex-nowrap">
        {!state.isLoading ? (
          state.categories.map((cat) => {
            ...
            return (
              <button
                ...
                className={`relative flex items-center gap-2 px-5 py-2.5 rounded-sm whitespace-nowrap transition-all flex-shrink-0 ${
          ...
          <div className="flex gap-2">
            {[1, 2, 3, 4, 5].map((i) => (
              <div key={i} className="h-10 w-24 rounded-sm bg-neutral-200 dark:bg-neutral-800 animate-pulse" />
            ))}
          </div>
        )}
      </div>
