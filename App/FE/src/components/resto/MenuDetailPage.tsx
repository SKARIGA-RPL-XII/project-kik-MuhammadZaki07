...
export default function MenuDetailPage() {
  ...
  // Logic Perhitungan Harga & Diskon (Tahan Error)
  const priceData = useMemo(() => {
    const originalPrice = menu?.price ?? 0;
    const discountValue = (menu?.discount?.is_active && menu?.discount?.value_discount) 
  ...
  if (isError || !menu) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50 p-6 text-center">
        <p className="text-slate-500 mb-4">Waduh, menu tidak ditemukan atau server lagi capek, Bos.</p>
        <button onClick={() => nav(-1)} className="text-indigo-600 font-bold underline">Kembali</button>
      </div>
    );
  }
  ...
  const handleAdd = () => {
    const isAllAttributesSelected = uniqueAttributes.every((attr: any) => selectedAttributes[attr.id]);
    if (!isAllAttributesSelected) {
      alert("Pilih semua opsi dulu ya!");
      return;
    }

    addToCart({ ...menu, selectedAttributes, finalPrice: priceData.discountedPrice }, quantity);
    nav(-1);
  };
  ...
  return (
    <div className="min-h-screen bg-white pb-40">
      <div className="relative h-[300px] overflow-hidden">
        <img src={imageUrl} className="absolute inset-0 w-full h-full object-cover" alt={menu.name} />
        <div className="absolute inset-0 bg-gradient-to-t from-white via-transparent to-transparent" />
        <button
          onClick={() => nav(-1)}
          className="absolute top-6 left-6 bg-white shadow-lg rounded-full p-3 active:scale-90 transition-transform"
        >
      ...
      <motion.div
        ...
        className="max-w-2xl mx-auto px-6 -mt-10 relative z-10"
      >
        <div className="bg-white rounded-t-[2.5rem] p-2">
          <div className="flex justify-between items-start mb-4">
            <div className="flex-1">
              ...
              <h1 className="text-2xl font-black text-slate-900 mt-2">{menu.name}</h1>
            </div>
          ...
          <p className="text-slate-500 text-sm mb-8 leading-relaxed">{menu.description}</p>

          <div className="space-y-8">
            {uniqueAttributes.length > 0 ? (
              uniqueAttributes.map((attr: any) => (
                <div key={attr.id}>
                  <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">{attr.name}</h3>
              ...
              <div className="p-4 bg-slate-50 rounded-2xl text-center">
                <p className="text-[10px] font-bold text-slate-400 uppercase">No extra options available</p>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 p-6 bg-white/90 backdrop-blur-md border-t border-slate-100">
        <div className="max-w-2xl mx-auto flex gap-4">
          ...
          <button
            onClick={handleAdd}
            className="flex-1 h-12 bg-indigo-600 text-white rounded-2xl font-black flex items-center justify-between px-6 text-xs shadow-lg shadow-indigo-600/20"
          >
            <div className="flex items-center gap-2">
              <ShoppingBag size={16} />
              <span>TAMBAH KE KERANJANG</span>
            </div>
            <span>Rp {(priceData.discountedPrice * quantity).toLocaleString("id-ID")}</span>
          </button>
        </div>
      </div>
    </div>
